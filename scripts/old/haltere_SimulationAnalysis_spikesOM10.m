...
% [tL, n, nS] = size( sim.Om10.strain  )
[tL, n, nS] = size( sim.Om10.strain(:, sidePoints,1) )
...
for j = 1:4
    strainTemp = sim.Om10.strain(:,sidePoints(j),1);
    t = (1:tL)/fSamp;
...
subplot(411); hold on
    plot( tS, sim.Om10.strain(:, sidePoints(2),1) ,'Color',col(1,:) )
    ax = gca();
    set(ax,axOpts{:})
    ylabel('Top Strain','Rotation',0); 
    sim.Om10.strain(:, sidePoints(2),1) 
     plot( tNon(spikeInds{2}) , zeros(length(spikeInds{2}),1)','ok')
subplot(412); hold on
    plot( tS, sim.Om10.strain(:, sidePoints(4),1),'Color',col(1,:) )
    ax = gca();
    set(ax,axOpts{:})
...
subplot(413); hold on
    plot( tS, sim.Om10.strain(:, sidePoints(1),1),'Color',col(2,:) )
    ax = gca();
    set(ax,axOpts{:})
...
subplot(414); hold on
    plot( tS, sim.Om10.strain(:, sidePoints(3),1),'Color',col(2,:) )
    ax = gca();
    set(ax,axOpts{:})
...
% Saving figure 
print(fig1, ['figs' filesep 'Figure_spikesFR' ], '-dpng', '-r600');
...
print(fig1, ['figs' filesep 'Figure_spikesFR' ], '-dsvg');
