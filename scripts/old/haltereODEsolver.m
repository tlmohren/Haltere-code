function [T,gammain,thetaout, phiout] = haltereODEsolver(factor,omega1,omega2,omega3,m1,m2,m3,R)
%%
...
if exist(['functions' filesep 'myODE_twist.m']) == 2
    display('PlateODE exists, deleting now ')
end
clear(['functions' filesep 'myODE_twist.m'])
...
iter =1; 
while exist(['functions' filesep 'myODE_twist.m'], 'file') ~= 2 && iter<5
    pause(2)
    iter = iter+1; 
end 
%% Solve ODEs
