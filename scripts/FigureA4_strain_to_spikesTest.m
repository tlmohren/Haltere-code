...
calib_param_max = [0.00298239604088481,0.0212443192274280,0.0372780009360082,0.0479350861876651,0.0514646550697229,0.0475135494972593,0.0363905581996612,0.0198191699014800,0.00299958384555981,0.0201797042231435,0.0370669398494605,0.0483996355340012,0.0524251480251299,0.0487363345563017,0.0377914840661745,0.0213792050886826];
    ...
    axCircle= { 'XLim',[0,t_plot(end)],'XTick',[0:0.001:t_plot(end)]}; 
    axZero= { 'XLim',[0,t_plot(end)],'XTick',[0:0.001:t_plot(end)],'XTickLabel',{'','',''}}; 
    axCircleSpike= { 'YLim',[0,1.6],'XLim',[0,t_plot(end)],'XTick',[0:0.001:t_plot(end)]}; 
    axZeroSpike= { 'YLim',[0,1.6],'XLim',[0,t_plot(end)],'XTick',[0:0.001:t_plot(end)],'XTickLabel',{'','',''}}; 
...
%     t_plot1 = (0:len1-1)*0.001;
    ...
    It10 = start10:(start10+len10-1);
    t_plot10 = (0:len10-1)*0.0001;
    t_plot10nonD = linspace(0,1,len10);
...
%         hold on
        for j = [1,2]%[1,2]+(2*(jj-1))
            ...
            if any(spike_ind_temp) & kl <=9
               plot( [1,1]'*t_plot10nonD(  spike_ind_temp(which_) - start10 ),...
...
%     plot( [1;1]*t_plot1, [-10;10]*ones(1,length(t_plot1)) ,'k')
...
%                    ([-3,10])  * ones(1, length(  spike_ind_temp(which_) )),':k')
    ...
    plot( [1;1]*t_plot1, [-10;10]*ones(1,length(t_plot1)) ,'k','LineWidth',0.1)
...
I_plot = 3000:3250; 
    for j = 1:2
        for kl = 1:length(selected_dots)% 1:length(FEA(j).circleInds)
            k = selected_dots(kl);
         tL = size(FEA(j).strain,2);
            strainTemp = FEA(j).strain( FEA(j).circleInds(k),:);
            t_plot = (1:tL)/fSamp;
            ...
            subplot(5,1,kl)
            plot(strainInterp(I_plot) )
            hold on
            
            [V,Imax(j,k)]= max(strainInterp(I_plot) );
            [V,Imin(j,k)]= min(strainInterp(I_plot) );
%             STA = STAfun(STAt);
%             strainConv = conv( [zeros(1,length(STA)-1),strainInterp], fliplr( STA), 'valid');
% %             strainConv = conv( [zeros(1,length(STA)-1),strainInterp], ( STA), 'valid');
%             calib_param(j,k) = max(  strainConv );
