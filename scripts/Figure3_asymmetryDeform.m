...
for j = [1,2,3]
    ...
    subplot(3,1,1); hold on 
        plot(t(It),FEA(j*2-1).yAngle(It) , lineSpec{j*2-1})
        plot(t(It),FEA(j*2).yAngle(It) , lineSpec{j*2})
        ylabel( labels{1} );
        ax = gca();
        set(ax,axOpts1{:})
        legend(legend_entries)
    subplot(312); hold on 
        plot(t(It),FEA(j*2-1).zAngle(It),  lineSpec{j*2-1} )
        plot(t(It),FEA(j*2).zAngle(It) , lineSpec{j*2} )
        ylabel( labels{2} );
        ax = gca();
        set(ax,axOpts2{:})
    subplot(313); hold on 
        plot(t(It),FEA(j*2-1).twistAngle(It),  lineSpec{j*2-1})
...
figure();hold on
        plot(t(It),FEA(j*2-1).twistAngle(It) - FEA(j*2).twistAngle(It) )
%         plot(t(It),FEA(j*2).twistAngle(It) , lineSpec{j*2})
        xlabel('Time (s)'); ylabel( labels{3} );
        ax = gca();
        set(ax,axOpts3{:})
...
print(fig1, ['figs' filesep 'Figure3_deformOffset' ], '-dpng', '-r600');
stupid_ratio = 15/16;
myfiguresize = [left, bottom, width*stupid_ratio, height*stupid_ratio];
set(fig1, 'PaperPosition', myfiguresize);
print(fig1, ['figs' filesep 'Figure3_deformOffset'], '-dsvg', '-r600');
...
% print(fig2, ['figs' filesep 'Figure3_deformMesh' ], '-dpng', '-r600');
% stupid_ratio = 15/16;
% myfiguresize = [left, bottom, width*stupid_ratio, height*stupid_ratio];
% set(fig2, 'PaperPosition', myfiguresize);
